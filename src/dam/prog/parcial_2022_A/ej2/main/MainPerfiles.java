...
public class MainPerfiles {
	...
	private static void printMenosContactos(Perfil[] perfiles) {
		...
		System.out.println("El perfil con menor n�mero de contactos es:\n" + peor);
	}
	...
	private static void solicitaPerfiles(Perfil[] perfiles) {
		...
		for (int i = 0; i < perfiles.length; i++) {
			System.out.printf("Introduce los datos del %d� perfil:\n", i);
			...
			try {
				...
				i--; // Para repetir esta iteraci�n del bucle
			}
		}
	}
	...
	public static String getString(String question, int minLen, int maxLen) {
		...
		while (true) {
			...
			if (str.length() < minLen) {
				System.out.println("La longitud m�nima es de " + minLen + " caracteres\n");
			}
			else if (str.length() > maxLen) {
				System.out.println("La longitud m�xima es de " + maxLen + " caracteres.\n");
			}
			else {
				return str;
			}
	...
	public static int getIntInRange(String question, int min, int max) {
		...
		while (isNotValid) {
			...
			if (n >= min && n <= max) {
				...
				System.out.printf(
					"El n�mero tiene que ser un natural en el rango [%d, %d]\n\n",
					min, max
				);
			}
		}
